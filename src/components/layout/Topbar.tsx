    ...
    <header className="h-14 flex items-center justify-between px-4 border-b border-border bg-(--bg-secondary)">
      <div className="flex items-center gap-2">
        <div className="md:hidden">
          <Sheet>
            ...
            />
            <SheetContent side="left" className="p-0 bg-(--bg-secondary) border-border w-72">
      ...
      <div className="flex items-center gap-3">
        <div className="hidden sm:block text-right">
          <div className="text-sm text-(--text-primary) font-medium">{user?.displayName ?? 'Agent'}</div>
          <div className="text-xs text-(--text-secondary)">{user?.email ?? ''}</div>
        </div>
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <Button variant="ghost" className="h-9 w-9 rounded-full p-0 hover:bg-(--bg-hover)">
          ...
          />
          <DropdownMenuContent align="end" className="bg-(--bg-card) border-border text-(--text-primary)">
            <DropdownMenuItem
